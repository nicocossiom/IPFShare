...
import { addEncryptedObject, createEncryptedTarFromPaths, decryptAndExtractTarball, downloadFromIpfs, getEnctrypedObject, uploadToIpfs } from "./ipfs/ipfsUtils.js";
...
const interactiveRegistryPrompt = async (promptMessage) => {
...
program.command("share")
    ...
    await withContext(async () => {
        ...
        const recipients = await interactiveRegistryPrompt("Select recipients");
...
const registryUpdateCommand = new Command("update")
    ...
    await withContext(async () => {
        ...
        if (!ctx.appConfig.user.username)
...
const registrySearchCommand = new Command("search")
    ...
    await withContext(async () => {
        ...
        console.log(await ctx.registry?.searchUsers((user) => {
