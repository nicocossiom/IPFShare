...
import { getAppConfigAndPromptIfUsernameInvalid } from "./appConfig.js";
...
export async function deInitializeContext() {
    ...
    if (!ctx.orbitdb)
        throw new Error("OrbitDB instance undefined, not closed yet, should not happen");
    ctx.identity?.provider.keystore.close();
    await ctx.orbitdb?.disconnect();
    ctx.daemonSocket?.write("resumeOrbitDB");
    process.exit(0);
}
function sendMessageToOrbitDBService(message) {
    return new Promise((resolve, reject) => {
        if (ctx.daemonSocket === undefined)
            throw new Error("Daemon socket is undefined");
