...
import { usernamePrompt } from "../setup.js";
import fs from "fs";
...
export async function getAppConfigAndPromptIfUsernameInvalid(daemon = false) {
    ...
    if (config.user.username === undefined
        ...
        if (!daemon) {
            ...
            if (!config.user.username)
                throw new Error("Username is not set and should have been set");
        }
    }
    if (!daemon) {
        if (!ctx.registry)
        ...
        if (!user)
            throw new Error("User is not found in the registry");
